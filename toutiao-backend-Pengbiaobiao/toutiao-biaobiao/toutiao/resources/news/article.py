...
from cache import statistic1 as cache_statistic
...
class ArticleListResource(Resource):
    ...
    def _feed_articles(self, channel_id, timestamp, feed_count):
        ...
        """
        # 创建用户进行rpc调用的工具
        stub = reco_pb2_grpc.UserRcommendStub(current_app.rpc_reco)
    ...
    def get(self):
        ...
        # 获取推荐文章列表
        # 使用rpc 请求推荐系统 获取该用户的推荐结果
        feeds, pre_timestamp = self._feed_articles(channel_id, timestamp, per_page)

        # 查询文章
        # feeds -> [包含文章id数据和文章埋点数据 ，与proto文件中的message Article对应]
        for feed in feeds:
            # 查询文章信息
